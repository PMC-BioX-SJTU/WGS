...
def Define_DellyMergeSV_Output():
	command=[
		'${bcftools} merge   \\\n',
                '\t-m id -O b   \\\n',
                '\t-o ${outpath4step}/${name}.merged.DUP.bcf   \\\n',
                '\t${inputtarget[@]} \n',

                '${bcftools} index     ${outpath4step}/${name}.merged.DUP.bcf\n',

                '${delly} filter -t DUP  \\\n',
                '-f germline  \\\n',
                '-o ${outpath4step}/${name}.germline.DUP.bcf  \\\n',
                '${outpath4step}/${name}.merged.DUP.bcf\n\n',

                '${bcftools} view  ${outpath4step}/${name}.germline.DUP.bcf > ${outpath4step}/${name}.germline.DUP.vcf\n\n'
