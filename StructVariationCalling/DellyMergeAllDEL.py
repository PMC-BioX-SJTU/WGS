...
def Define_DellyMergeSV_Output():
	command=['${bcftools} merge   \\\n',
                '\t-m id -O b   \\\n',
                '\t-o ${outpath4step}/${name}.merged.DEL.bcf   \\\n',
                '\t${inputtarget[@]} \n',

                '${bcftools} index     ${outpath4step}/${name}.merged.DEL.bcf\n',

                '${delly} filter -t DEL  \\\n',
                '-f germline  \\\n',
                '-o ${outpath4step}/${name}.germline.DEL.bcf  \\\n',
                '${outpath4step}/${name}.merged.DEL.bcf\n\n',

                '${bcftools} view  ${outpath4step}/${name}.germline.DEL.bcf > ${outpath4step}/${name}.germline.DEL.vcf\n\n'
