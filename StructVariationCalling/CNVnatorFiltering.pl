...
# CNVnator
open OUT, ">$output4step/$name.filtered.tab" or die;
open C, "<$output4step/$name.cnvnator" or die;
while (<C>) {
	my $line = $_;
	if ($line && ($params{cnvnator_P} || $params{cnvnator_q0} || $params{cnvnator_RDcomplex})) {
		$line = $parse->filterCNVnator($line,$params{cnvnator_P},$params{cnvnator_q0},$params{cnvnator_RDcomplex});
	}
	# Reformat to tab-delimeted and BED formats
	if ($line) {
		$line = $parse->cnvnator2tab($line,$name);
